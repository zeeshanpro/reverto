                ...
                <div class="col-md-5">
                    ...
                    <div class="form-group all">
                        ...
                        <div class="input-group">
                            <?= form_input('code', ($_POST['code'] ?? ($product ? $product->code : '')), 'class="form-control' . ($Settings->use_code_for_slug ? ' gen_slug' : '') . '" id="code"') ?>
                            <span class="input-group-addon pointer" id="random_num" style="padding: 1px 10px;">
                                <i class="fa fa-random"></i>
                            </span>
                            <span class="input-group-addon pointer" id="add_barcode" style="padding: 1px 10px;">
                                <i class="fa fa-plus"></i>
                            </span>
                        </div>
                        <span class="help-block"><?= lang('you_scan_your_barcode_too') ?></span>
                        <input type="hidden" class="form-control" name="product_codes" data-role="tagsinput" id="product_codes">
                        <ol id="barcodeValues"></ol>
                    </div>
                    <div class="form-group all">
                        <?= lang('slug', 'slug'); ?>
                    ...
                    <div class="form-group" style="display:none;">
                        <?= lang('quantity', 'quantity') ?>
                         <?= form_input('quantity', ($_POST['quantity'] ?? ($product ? $this->sma->formatQuantityDecimal($product->quantity) : '')), 'class="form-control tip" id="quantity"') ?>
                    </div>
                    <div class="form-group" style="display:none;">
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        let barcodeLists = [];
        $(document).on('click', '#add_barcode', function () {
            if($.trim($('#code').val()) == ''){
            ...
            else
            {
                ...
                if(barcodeLists.indexOf(barcode) !== -1)
                {
                    alert("Already Exist, Please Add Unique Codes");
                }
                else
                {
                     barcodeLists.push(barcode);
                     $('#product_codes').val(barcodeLists);
                }
                
                var barcodevalues = document.getElementById("barcodeValues");
                $("#barcodeValues").html('');
                for(var i=0; i<barcodeLists.length; i++)
                    barcodevalues.innerHTML += "<li>Barcode ["+i+"]: "+barcodeLists[i]+"</li>";
                $('#code').val('');
            }
        });
