...
class Products extends MY_Controller
{
    ...
    public function add($id = null)
    {
        ...
        $this->form_validation->set_rules('product_codes', lang('product_codes'), 
            'required');
        if (SHOP) {
        ...
        if ($this->form_validation->run() == true) {
            $all_product_codes = explode(",", $this->input->post('product_codes'));
            ...
            foreach($all_product_codes as $product_code)
            {
                ...
                $data     = [
                    ...
                    'slug'              => $this->input->post('slug').'-'.$product_code,
                ...
                $data['quantity'] =  $data['quantity'] ?? $wh_total_quantity ;
    ...
    //     } else {
    //         $this->datatables
    //             ->select($this->db->dbprefix('products') . ".id as productid, {$this->db->dbprefix('products')}.image as image, {$this->db->dbprefix('products')}.code as code, {$this->db->dbprefix('products')}.name as name, {$this->db->dbprefix('brands')}.name as brand, {$this->db->dbprefix('categories')}.name as cname, cost as cost, price as price, count(*) as quantity, {$this->db->dbprefix('units')}.code as unit, '' as rack, alert_quantity", false)
    //             ->from('products')
    //             ->where('products.quantity IS NOT NULL')
        ...
        public function getProducts($warehouse_id = null)
    {
        ...
        $this->datatables->add_column('Actions', $action,'product_count',$dis_products,'productid, image, code, name');
        echo $this->datatables->generate();
    }

public function getDistinctProducts($warehouse_id = null)
    {
        $this->sma->checkPermissions('index', true);
        $supplier = $this->input->get('supplier') ? $this->input->get('supplier') : null;
        ...
        if ($warehouse_id) {
            $this->datatables
            ->select($this->db->dbprefix('products') . ".id as productid, {$this->db->dbprefix('products')}.image as image, {$this->db->dbprefix('products')}.name as name, {$this->db->dbprefix('brands')}.name as brand, {$this->db->dbprefix('categories')}.name as cname, cost as cost, price as price, COALESCE(wp.quantity, 0) as quantity, {$this->db->dbprefix('units')}.code as unit, wp.rack as rack, alert_quantity", false)
            ->from('products');
            if ($this->Settings->display_all_products) {
                $this->datatables->join("( SELECT product_id, quantity, rack from {$this->db->dbprefix('warehouses_products')} WHERE warehouse_id = {$warehouse_id}) wp", 'products.id=wp.product_id', 'left');
            } else {
        ...
        // ->group_by("products.id");
            ...
            $this->datatables
                ->select($this->db->dbprefix('products') . ".id as productid, {$this->db->dbprefix('products')}.image as image, {$this->db->dbprefix('products')}.name as name, {$this->db->dbprefix('brands')}.name as brand, {$this->db->dbprefix('categories')}.name as cname, cost as cost, price as price, count(*) as quantity, {$this->db->dbprefix('units')}.code as unit, '' as rack, alert_quantity", false)
                ->from('products')
                ->where('products.quantity is NOT NULL')
                ->join('categories', 'products.category_id=categories.id', 'left')
                ->join('units', 'products.unit=units.id', 'left')
        ...
        $this->datatables->add_column('product_count',$dis_products,'productid, image, code, name');
        echo $this->datatables->generate();
    }
    ...
    public function group_index($warehouse_id = null)
    {
        $this->sma->checkPermissions();
        ...
        $bc                     = [['link' => base_url(), 'page' => lang('home')], ['link' => '#', 'page' => lang('products')]];
        $meta                   = ['page_title' => lang('products'), 'bc' => $bc];
        $this->page_construct('products/distinct_product_list', $meta, $this->data);
    }
